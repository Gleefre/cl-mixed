...
(defmethod initialize-instance :after ((segment virtual) &key)
  (let ((handle (handle segment)))
    (setf (mixed-cffi:direct-segment-free handle) (cffi:callback virtual-free))
    (setf (mixed-cffi:direct-segment-info handle) (cffi:callback virtual-info))
    (setf (mixed-cffi:direct-segment-start handle) (cffi:callback virtual-start))
    (setf (mixed-cffi:direct-segment-mix handle) (cffi:callback virtual-mix))
    (setf (mixed-cffi:direct-segment-end handle) (cffi:callback virtual-end))
    (setf (mixed-cffi:direct-segment-set-in handle) (cffi:callback virtual-set-input))
    (setf (mixed-cffi:direct-segment-set-out handle) (cffi:callback virtual-set-output))
    (setf (mixed-cffi:direct-segment-get-in handle) (cffi:callback virtual-get-input))
    (setf (mixed-cffi:direct-segment-get-out handle) (cffi:callback virtual-get-output))
    (setf (mixed-cffi:direct-segment-set handle) (cffi:callback virtual-set))
    (setf (mixed-cffi:direct-segment-get handle) (cffi:callback virtual-get))))
...
(define-callback virtual-info :pointer ((segment :pointer))
  ...
  (destructuring-bind (&key name description flags min-inputs max-inputs outputs fields)
    ...
    (let ((info (cffi:foreign-alloc '(:struct mixed-cffi:segment-info))))
      (setf (mixed-cffi:segment-info-name info) name)
      (setf (mixed-cffi:segment-info-description info) description)
      (setf (mixed-cffi:segment-info-flags info) flags)
      (setf (mixed-cffi:segment-info-min-inputs info) min-inputs)
      (setf (mixed-cffi:segment-info-max-inputs info) max-inputs)
      (setf (mixed-cffi:segment-info-outputs info) outputs)
